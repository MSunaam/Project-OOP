...
AppliedPhysics::AppliedPhysics(int loginID, int password, int mins = 0, string pass = "ABCD")
	:Teacher(loginID, password, mins, pass) {
	writePass();
	writeTimer();
}
void AppliedPhysics::writePass() const {
	if (getPass() != "ABCD") {
		fstream file("AppliedPhysics.txt");
		file << getPass() << endl;
		file.close();
	}
}
void AppliedPhysics::writeTimer() const {
	if (getTimer() != 0) {
		fstream file("AppliedPhysics.txt");
...
void AppliedPhysics::addQues() {
	int num;
	string question;
	string A, B, C, D;
	cout << "How many questions do you want to add?" << endl;
	cin >> num;
	fstream file("AppliedPhysics.txt", ios::app);
